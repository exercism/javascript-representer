...
describe('DirectoryInput', () => {
  describe('a valid submission with extra files', () => {
    const SOLUTION_FILES = {
      ...
      '/path/to/solution/two-fer.spec.js':
        'describe("a test", () => { it("is true") { expect(true).toBe(true) }}})',
    ...
    test('it excludes files that are .spec.js', async () => {
      ...
      expect(files).not.toContain(
        SOLUTION_FILES['/path/to/solution/two-fer.spec.js']
      )
    })
  ...
  describe('a valid submission with all files', () => {
    const SOLUTION_FILES = {
      '/path/to/solution/.exercism/metadata.json':
        '{"track":"javascript","exercise":"two-fer"}',
      '/path/to/solution/node_modules/@babel/cli/index.js':
        'throw new Error("Use the `@babel/core` package instead of `@babel/cli`.")',
      '/path/to/solution/.eslintrc': '{ "extends": [ "eslint: recommended" ] }',
      '/path/to/solution/babel.config.js':
        'module.exports = { presets: [ "@babel/env" ] }',
      '/path/to/solution/.eslintrc.js':
        'module.exports = { extends: [ "eslint: recommended" ] }',
      '/path/to/solution/jest.config.js':
        'module.exports = { "transform": { "^.+\\.[jt]sx?$"": "babel-jest" } }',
      '/path/to/solution/package-lock.json':
    ...
    test('it excludes files that are not .js', async () => {
      ...
      expect(files).not.toContain(SOLUTION_FILES['/path/to/solution/README.md'])
      expect(files).not.toContain(
        SOLUTION_FILES['/path/to/solution/package.json']
      )
      expect(files).not.toContain(
        SOLUTION_FILES['/path/to/solution/package-lock.json']
      )
      expect(files).not.toContain(SOLUTION_FILES['/path/to/solution/.eslintrc'])
      expect(files).not.toContain(
        SOLUTION_FILES['/path/to/solution/.exercism/metadata.json']
      )
    })
    ...
    test('it excludes files that are configuration', async () => {
      ...
      expect(files).not.toContain(
        SOLUTION_FILES['/path/to/solution/.eslintrc.js']
      )
      expect(files).not.toContain(
        SOLUTION_FILES['/path/to/solution/babel.config.js']
      )
      expect(files).not.toContain(
        SOLUTION_FILES['/path/to/solution/jest.config.js']
      )
    })
